...
vector<double> Vehicle::state_in(double time){
  ...
  */
  ...
  vector<double> state = {
        s[0] + (s[1]*time) + pow(s[2]*time, 2) /2.0,
        s[1] + s[2]*time,
        s[2],
        d[0] + (d[1]*time) + pow(d[2]*time, 2)/2.0,
        d[1] + d[2]*time,
        d[2]
  };
  return state;
...
vector<double> Vehicle::get_target_for_state(string state, 
  ...
  leading_vehicle_s_and_sdot = get_leading_vehicle_data_for_lane(target_lane, other_cars, duration);
  ...
  if (leading_vehicle_s - target_s < FOLLOW_DISTANCE && leading_vehicle_s > this->s) {
    ...
    if (fabs(leading_vehicle_s - target_s) < 0.5 * FOLLOW_DISTANCE) {
      target_s_d -= 1; // slow down if too close by velocity -1
    }
...
vector<double> Vehicle::get_leading_vehicle_data_for_lane(int target_lane, 
                vector<Vehicle> other_cars, double duration) {
...
Trajectory Vehicle::generate_traj_for_target(vector<double> target_state, double duration) {
  ...
  vector<double> target_s_coeffs = {target_state[0], target_state[1], target_state[1]};
  vector<double> target_d_coeffs = {target_state[3], target_state[4], target_state[5]};
  vector<double> current_s_coeffs = {this->s, this->s_d, this->s_dd};
  vector<double> current_d_coeffs = {this->d, this->d_d, this->d_dd};

  // determine coefficients of optimal JMT 
  // jmt return the  6 polynomial coefficents
  this->s_traj_coeffs = jmt(current_s_coeffs, target_s_coeffs, duration); 
  this->d_traj_coeffs = jmt(current_d_coeffs, target_d_coeffs, duration);
